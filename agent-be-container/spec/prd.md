...
- **Robust MFE Parsing**: Tools that accept `MFEContent` (like `add_visualization`) include a robust parsing layer to handle various LLM output formats, including raw dictionaries, JSON strings, and Python-like object representations (reprs), ensuring reliable state updates even when the LLM deviates from strict JSON schemas.
