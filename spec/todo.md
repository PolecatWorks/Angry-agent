...
3.  [x] **`generate_mfe_of_text`**
    *   [x] Test happy path: Pass a `TextInput` with a plain string. Verify the returned `MFEContent` has `provider="mfe1"`, `component="./TextShowWrapper"`, and the content matches the input text.

4.  [x] **`generate_mfe_of_personal_data_form`**
    *   [x] Test happy path with all fields populated: Construct a `PersonalDataForm` with first_name, last_name, email, phone_number, and address. Pass it in `PersonalDataFormInput`. Verify the returned `MFEContent` uses component `./PersonalDataFormWrapper` and preserves all fields.
    *   [x] Test happy path with partial fields: Construct a `PersonalDataForm` leaving optional fields as `None`. Verify that the tool handles this and returns an `MFEContent` object with those missing fields intact as `None`.
...
7.  [x] **`browse_visualizations`**
    *   [x] Test happy path: Pass a mock state containing several `MFEContent` items. Verify the tool returns the exact list of visualizations from the state.
    *   [x] Test empty state: Pass a mock state with an empty list of visualizations. Verify it returns an empty list.

8.  [x] **`read_visualization`**
    *   [x] Test happy path: Pass a valid ID that exists in the mocked state. Verify the tool returns the specific `MFEContent` object.
    *   [x] Test missing ID: Pass an ID that does not exist in the mocked state. Verify the tool raises a `ToolException` with the correct error message.
...
10. [x] **`add_visualization`**
    *   [x] Test happy path: Pass a brand new `MFEContent` object and a dummy `tool_call_id`. Verify the function returns a `Command` with `"action": "add"` in the visualizations update list, and includes the correct `ToolMessage`.
