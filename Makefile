...
export APP_PACKAGER_AICLIENT__GOOGLE_API_KEY := ${GOOGLE_API_KEY}
export APP_EMBEDDING_CLIENT__GOOGLE_API_KEY := ${GOOGLE_API_KEY}
...
$(foreach app,$(APPS),$(app)-docker):%-docker:
	cd $*-container && docker build ${$*_DOCKER_BUILD_OPTS} -t $* .
